...
export default async function Image({ params }: { params: Promise<{ token: string }> }) {
  ...
  const name: string = kitten?.name ?? "Kitten";
  const photoUrl: string | null = kitten?.photo ?? null;
  ...
  const subtitle = [ageLabel, sex].filter(Boolean).join(" · ");
  ...
  // Fetch kitten photo as base64 so satori doesn't need to resolve external URLs
